...
die "figure out when the software was created, if no vagrant VM
already exists, grab disks into linux-media-acquisition for a distro
capable of building it from that time, install required software and
build, move executable or package where needed\n";
